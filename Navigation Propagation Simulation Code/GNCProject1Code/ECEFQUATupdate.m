function C_ECEF_QUAT = ECEFQUATupdate(t,C,w_e_v,u,r_e,J2)
...
q_BodytoFixed = [C(1); C(2); C(3); C(4)];

C_BodytoFixed = [C(1)^2+C(2)^2-C(3)^2-C(4)^2 2*(C(2)*C(3)-C(1)*C(4)) 2*(C(1)*C(3)+C(2)*C(4));
                 2*(C(2)*C(3)+C(1)*C(4)) C(1)^2-C(2)^2+C(3)^2-C(4)^2 2*(C(3)*C(4)-C(1)*C(2));
                 2*(C(2)*C(4)-C(1)*C(3)) 2*(C(1)*C(2)+C(3)*C(4)) C(1)^2-C(2)^2-C(3)^2+C(4)^2];
             
we = wb-[0; ((C_BodytoFixed)'*w_e_v)];

We = [we(1) -we(2) -we(3) -we(4);
      we(2)  we(1) we(4) -we(3)
      we(3) -we(4) we(1)  we(2)
      we(4)  we(3) -we(2) we(1)];

K = 0.5*We*q_BodytoFixed;

q_ECEF_QUAT = [K(1); K(2); K(3); K(4)];
...
C_ECEF_QUAT = [q_ECEF_QUAT; r_d; v_d];
