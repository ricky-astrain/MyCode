...
xlabel('x'),ylabel('G(x)'),title('First 5 Eigen Functions for Part A')
...
figure(2)
plot(x,u)
xlabel('x'),ylabel('u(x,t)'),title('Solution for Part A')
legend('t=0','t=0.01','t=0.05','t=0.2')
%% Part B
% -- costruct D
D = 50.*x;
% -- construct the main matrix for the eigenvalue problem 
...
figure(3)
plot(x,vs(:,1),x,vs(:,2),x,vs(:,3),x,vs(:,4),x,vs(:,5))
xlabel('x'),ylabel('G(x)'),title('First 5 Eigen Functions for Part B')
legend('1st eigf','2nd eigf','3rd eigf','4th eigf','5th eigf','Location','SouthEast')
% -- construct P(x) and determine an and u(x,t)
t = [0 0.01 0.05 0.2];
for i = 1:length(x)
...
figure(4)
plot(x,u)
xlabel('x'),ylabel('u(x,t)'),title('Solution for Part B')
legend('t=0','t=0.01','t=0.05','t=0.2')
